...
static const PAL_BITMAP_CONTEXT _apPaletteBitmaps_Bk[] = {
   { &bmhouse_smile,     _Colorshouse_smile,     _aPalettesHouse_Bk,   GUI_COUNTOF(_Colorshouse_smile)  },
};

#if 0
GUI_CONST_STORAGE GUI_BITMAP *_NumAqiIndex[] = {
  &bm1, &bm2, &bm3,
};

static const PAL_BITMAP_CONTEXT _apPaletteNum1[] = {
   { &bm2,     _Colors2,     _aPalettesHouse,     GUI_COUNTOF(_Colors2)   },
};

static const PAL_BITMAP_CONTEXT _apPaletteNum1_Bk[] = {
   { &bm2,     _Colors2,     _aPalettesHouse_Bk,   GUI_COUNTOF(_Colors2)  },
};
#endif
/*********************************************************************
*
...
static void _cbWin0(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
    xSize = LCD_GetXSize();
    ySize = LCD_GetYSize();
	//GUI_DrawBitmap(&bmhouse,(anim_xpos- bmhouse.XSize) / 2, NULL);
    break;
  }
}
#endif
...
*/
#if 0
int NumAqiAnimatiom(void)
{
	unsigned i, j;
	unsigned NumPalBitmaps;
	int      xSize, ySize;
	
	NumPalBitmaps = GUI_COUNTOF(_apPaletteNum1);
	xSize = LCD_GetXSize();
	ySize = LCD_GetYSize();
	...
	for (j = NUM_STEPS_PAL; j > 0; j--) {
	  	_SetPalette(_apPaletteNum1, NumPalBitmaps, i, 255 * j / NUM_STEPS_PAL);
		GUI_DrawBitmap(&bm2, (xSize  - bm2.XSize) / 2 + FACE_OFFSET, HOUSE_FACE_YPOS);
		GUI_Delay(90);
	}
	}
	...
	for (j = NUM_STEPS_PAL; j > 0; j--) {
	  	_SetPalette(_apPaletteNum1_Bk, NumPalBitmaps, i, 255 * j / NUM_STEPS_PAL);
		GUI_DrawBitmap(&bm2,  (xSize  - bm2.XSize) / 2 + FACE_OFFSET, HOUSE_FACE_YPOS);
	  	GUI_Delay(90);
	}
	}
...
int NormalWorkModeAnimation(void) {
	...
	for (i = 0; i < GUI_COUNTOF(_aPalettesHouse_Bk) - 1; i++) {
	for (j = NUM_STEPS_PAL; j > 0; j--) {
	  	_SetPalette(_apPaletteBitmaps_Bk, NumPalBitmaps, i, 255 * j / NUM_STEPS_PAL);
		GUI_DrawBitmap(&bmhouse_smile,  (xSize  - bmhouse_smile.XSize) / 2 + FACE_OFFSET, HOUSE_FACE_YPOS);
	  	GUI_Delay(90);
	}
	}
...
int Face_Appear_Fading_Smile(const GUI_BITMAP *pBM, int x_offset, int y_offset)
{
	unsigned i, j;
	
	for (i = 0; i < GUI_COUNTOF(_aPalettesHouse) - 1; i++) {
	for (j = NUM_STEPS_PAL; j > 0; j--) {
		_SetPalette(_HouseSmilePaletteBitmaps, GUI_COUNTOF(_HouseSmilePaletteBitmaps), i, 255 * j / NUM_STEPS_PAL);
	...
	for (i = 0; i < GUI_COUNTOF(_aPalettesHouse_Bk) - 1; i++) {
	for (j = NUM_STEPS_PAL; j > 0; j--) {
		_SetPalette(_HouseSmilePaletteBitmaps_Bk, GUI_COUNTOF(_HouseSmilePaletteBitmaps_Bk), i, 255 * j / NUM_STEPS_PAL);
		GUI_DrawBitmap(pBM,  (LCD_GetXSize()  - pBM->XSize) / 2 + x_offset, (LCD_GetYSize()  - pBM->YSize) / 2 + y_offset);
		GUI_Delay(90);
	}
	}
