...
typedef struct {
	 unsigned char bChanged      ;  // 数据更新标志
		/****************** bChanged 标志位说明 ************************
			...
			sbit4 :VOC据据改变
		****************** bChanged 标志位说明 ************************/
   	 unsigned long iPM2005        ; //	ug/m3
	 long iHumidity      ; // 常用1%表示，此处采用 1%00
	 long iTemperature   ;	// 常用度表示，此处采用 1%度
	 unsigned long iCO2  ;	// 1ppm=1mg/kg=1mg/L=1×10-6
	 unsigned long iVOC  ;	//待定
