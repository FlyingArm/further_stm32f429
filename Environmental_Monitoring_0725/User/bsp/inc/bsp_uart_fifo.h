...
/* 定义使能的串口, 0 表示不使能（不增加代码大小）， 1表示使能 */
	...
	*/
	...
	/* PM2005使能GPIO, PC9 */
	#define RCC_PM2005_EN	RCC_AHB1Periph_GPIOA
	#define PORT_PM2005_EN  GPIOA
	#define PIN_PM2005_EN	GPIO_Pin_2

	#define PM2005_EN()		PORT_PM2005_EN->BSRRH = PIN_PM2005_EN
	#define PM2005_OFF()	PORT_PM2005_EN->BSRRL = PIN_PM2005_EN
	
	/* CO2使能GPIO, A1 */
	#define RCC_CO2_EN		RCC_AHB1Periph_GPIOC
	#define PORT_CO2_EN  	GPIOC
	#define PIN_CO2_EN	 	GPIO_Pin_11

	#define CO2_EN()	  	PORT_CO2_EN->BSRRH = PIN_CO2_EN
	#define CO2_OFF()	 	PORT_CO2_EN->BSRRL = PIN_CO2_EN
#endif
...
#if UART2_FIFO_EN == 1
	#define UART2_BAUD			115200 // no use
	#define UART2_TX_BUF_SIZE	1*64
	#define UART2_RX_BUF_SIZE	1*128
#endif

#if UART3_FIFO_EN == 1
	#define UART3_BAUD			115200 // wifi
	#define UART3_TX_BUF_SIZE	1*1024
	#define UART3_RX_BUF_SIZE	1536
#endif

#if UART4_FIFO_EN == 1
	#define UART4_BAUD			9600 	// CO2 sensor baut:9600
	#define UART4_TX_BUF_SIZE	1*64
	#define UART4_RX_BUF_SIZE	1*128
#endif

#if UART5_FIFO_EN == 1
	#define UART5_BAUD			9600	// pm2.5 sensor baut:9600
	#define UART5_TX_BUF_SIZE	1*64
	#define UART5_RX_BUF_SIZE	1*128
#endif

#if UART6_FIFO_EN == 1
	#define UART6_BAUD			115200	// bluetooth
...
void CO2_sensor_data_rcv(void);
void PM2005_sensor_data_rcv(void);
void calibration_command_rcv(void);

int comCalcRxFifo(COM_PORT_E _ucPort);
