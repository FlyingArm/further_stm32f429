...
void ili9806g_lcd_init(void)
{
	...
	OSTimeDly(50, OS_OPT_TIME_DLY, &err); 						//延时一会 
	LCD_Nreset_On;
	OSTimeDly(110, OS_OPT_TIME_DLY, &err); 
	LCD_Nreset_Off;						//液晶屏复位
	OSTimeDly(150, OS_OPT_TIME_DLY, &err); 					//延时一会 
	LCD_Nreset_On;
	OSTimeDly(200, OS_OPT_TIME_DLY, &err); 	//延时一会 
