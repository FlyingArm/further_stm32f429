...
/* ???RST?INT?? */ 
#define FT5X06_RST_1() GPIO_SetBits(PORT_TP_RST,PIN_TP_RST);	// PORT_TP_RST->BSRRL = PIN_TP_RST	/* RST = 1 */ 
#define FT5X06_RST_0() GPIO_ResetBits(PORT_TP_RST,PIN_TP_RST);  // PORT_TP_RST->BSRRH = PIN_TP_RST	/* RST = 0 */ 

#define FT5X06_INT_1() GPIO_SetBits(PORT_TP_INT,PIN_TP_INT);   //PORT_TP_INT->BSRRL = PIN_TP_INT	/* INT = 1 */ 
#define FT5X06_INT_0() GPIO_ResetBits(PORT_TP_INT,PIN_TP_INT); //PORT_TP_INT->BSRRH = PIN_TP_INT	/* INT = 0 */ 


static void FT5X06_ReadReg(uint16_t _usRegAddr, uint8_t *_pRegBuf, uint8_t _ucLen);
static uint8_t FT5X06_WriteReg(uint8_t addr,uint16_t _usRegAddr, uint8_t *_pRegBuf, uint8_t _ucLen) ;
...
{	
	...
	FT5X06_WriteReg(0xBA,0x8040, (uint8_t *)buf, 1); 
...
void FT5X06_OnePiontScan(void)
{
  uint8_t buf[40]; 
	...
	if ((buf[0] & 0x0F) == 0)
	{
		...
		FT5X06_WriteReg(0xBA,0x814E, (uint8_t *)&Clearbuf, 1); 
		return;
	}
	FT5X06_ReadReg(0x814E+1, &buf[1], 39); 
	FT5X06_WriteReg(0xBA,0x814E, (uint8_t *)&Clearbuf, 1); 
	

	g_tFT5X06.X[0] = ((uint16_t)buf[3] << 8) + buf[2]; 
	g_tFT5X06.Y[0] = ((uint16_t)buf[5] << 8) + buf[4]; 
	
	g_tFT5X06.X[1] = ((uint16_t)buf[11] << 8) + buf[10]; 
	g_tFT5X06.Y[1] = ((uint16_t)buf[13] << 8) + buf[12]; 
	
	g_tFT5X06.X[2] = ((uint16_t)buf[19] << 8) + buf[18]; 
	g_tFT5X06.Y[2] = ((uint16_t)buf[21] << 8) + buf[20]; 
	
	g_tFT5X06.X[3] = ((uint16_t)buf[27] << 8) + buf[26]; 
	g_tFT5X06.Y[3] = ((uint16_t)buf[29] << 8) + buf[28]; 
	
	g_tFT5X06.X[4] = ((uint16_t)buf[35] << 8) + buf[34]; 
	g_tFT5X06.Y[4] = ((uint16_t)buf[37] << 8) + buf[36]; 
	...
	/* 检测按下 */
	{
		...
		else if (g_tFT5X06.ChipID == 0x64)	/* 5.0寸 854 * 480 */
		{
			x = g_tFT5X06.X[0];
			y = g_tFT5X06.Y[0];	
		...
		else if (g_tFT5X06.ChipID == 0x39)	/* 5.0寸 854 * 480 */
		{
			x = g_tFT5X06.X[0];
			y = g_tFT5X06.Y[0];	
...
uint8_t FT5X06_ReadID(void)
{
	...
	FT5X06_ReadReg(/*FTS_REG_CHIP_ID*/0x8140, &id, 1);
