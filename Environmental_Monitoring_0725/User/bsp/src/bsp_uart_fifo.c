...
#define UART2_TX_SOURCE    GPIO_PinSource1

#define UART2_RX_PORT      GPIOA
#define UART2_RX_PIN       GPIO_Pin_0
#define UART2_RX_CLK       RCC_AHB1Periph_GPIOA
#define UART2_RX_SOURCE    GPIO_PinSource0
...
#define UART6_RX_CLK       RCC_AHB1Periph_GPIOG
#define UART6_RX_SOURCE    GPIO_PinSource9
...
int comCalcRxFifo(COM_PORT_E _ucPort)
{
	...
	if((pUart->usRxWrite > 0) && (pUart->usRxCount > 0))
	{
...
static void InitHardUart(void)
{
	...
	*/
	...
	//USART_ITConfig(USART3,USART_IT_IDLE,ENABLE); /* IDLE 中断 */
	/*
		USART_ITConfig(USART1, USART_IT_TXE, ENABLE);
		注意: 不要在此处打开发送中断
		发送中断使能在SendUart()函数打开
	...
	*/
	...
	/* 将 PG14 映射为 USART6_RX */
	GPIO_PinAFConfig(GPIOG, GPIO_PinSource14, GPIO_AF_USART6);

	/* 将 PG9 映射为 USART6_TX */
	GPIO_PinAFConfig(GPIOG, GPIO_PinSource9, GPIO_AF_USART6);

	/* 配置 PG14/USART6_RX 为复用功能 */
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;	/* 输出类型为推挽 */
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;	/* 内部上拉电阻使能 */
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;	/* 复用模式 */
